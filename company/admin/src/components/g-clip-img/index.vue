<template>
    <div class="g-clip-img" :style="`width: ${props.width}px;`">
        <!-- 图片裁剪组件 -->
...
const imgLocal = ref('https://gem530.github.io/img/33.jpg')
...
const imgInit = () => {
    return new Promise((resolve, reject) => {
        fabric.Image.fromURL(imgLocal.value, (img: any) => {
            imgValue.value = img
            imgSizeInit()
            resolve(img)
...
const mouseWheelHandle = () => {
    canvasRef.value.on("mouse:wheel", function(ev: any) {
        const { e } = ev
        e.preventDefault()
...
// 图片上传
const handleChangeUploadImg = (ev: any) => {
    const { target } = ev
    ...
    reader.onload = function(e: any) {
        ...
        imgValue.value.setSrc(imgLocal.value, (img: any) => {
            imgValue.value = img
            imgStyleInit()
            imgSizeInit()
            canvasRef.value.renderAll()
...
// 用于产看压缩后的图片大小
const convertBase64UrlToBlob = (urlData: string|any) => {
