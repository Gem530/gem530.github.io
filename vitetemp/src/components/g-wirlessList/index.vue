...
<script setup lang="ts">
    import { watch, onMounted, nextTick, computed, defineProps, defineEmits, defineExpose } from 'vue'
    ...
    const showList = computed(() => {
        return props.list.slice(state.start, state.start + state.showNum)
    })
    ...
    onMounted(() => {
        ...
        state.showNum = Math.round(h / props.itemHeight) + 3
        scroll()
    ...
    // 初始化高度
    const initHeight = () => {
        scrollBar.value.style.height = props.list.length * props.itemHeight + 'px'
    }

    // 滚动时，控制数组盒子位置
    const scroll = () => {
        const scrollTop = Math.round(box.value.scrollTop)
        state.start = Math.round(scrollTop / props.itemHeight)
        scrollList.value.style.top = scrollTop + 'px'
    }
    ...
    const touchstart = (e: any) => {
        e = e || window.event
        if (!state.isMobile && !box.value.contains(e.target)) {
            return
        }
        // console.log('手指接触屏幕')
