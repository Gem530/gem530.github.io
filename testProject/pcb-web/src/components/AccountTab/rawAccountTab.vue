<template>
  <el-tabs v-model="tabRadioTable" @tab-click="tabRadioTableHandle" type="border-card" class="ptable-tab">
      ...
      <XTable :pageShow="true" v-model:page-size="tabQueryParams.pageSize" :toolId="props.tabToolId1?props.tabToolId1:'tabToolId1'"
        ...
        <template #edit-quantity="{ row }">
          <el-input-number
            ...
            maxLength="11" :min="0" :max="(row.inOutQuantity||row.inOutQuantity==0) ? row.inOutQuantity : row.outQuantity" :precision="props.tabQuantity1?props.tabQuantity1:0"
            style="width: 99%;" @change="calculatePrice(row)" :controls="false" v-model="row.quantity" placeholder="请输入数量" />
        </template>
        ...
        <template #edit-price="{ row }">
          <el-input-number
            :disabled="props.drawerBorrow?.title?.includes('详情') || props.drawerBorrow?.title?.includes('审核')"
            :min="0"  maxLength="11" :precision="4" style="width: 99%;" @change="calculatePrice(row)" :controls="false"
            v-model="row.price" placeholder="请输入单价" />
        </template>
        ...
        <template #edit-discountPrice="{ row }">
          <el-input-number
            :disabled="props.drawerBorrow?.title?.includes('详情') || props.drawerBorrow?.title?.includes('审核')" :min="0"
            maxLength="11" :precision="2" style="width: 99%;" @change="calculatePrice(row)" :controls="false"
            v-model="row.discountPrice" placeholder="请输入金额" />
        </template>
        <template #default-remark="{ row }">
          {{ row.remark }}
        </template>
        <template #edit-remark="{ row }">
          <el-input :disabled="props.drawerBorrow?.title?.includes('详情') || props.drawerBorrow?.title?.includes('审核')"
            maxLength="100" style="width: 99%;" v-model="row.remark" @change="changeOrderListHandle(row)" />
        </template>
      </XTable>
    </el-tab-pane>
    ...
    <el-tab-pane v-if="props.tabName2" :label="props.tabName2 + '(' + (props.allData?.tabTotal2?props.allData?.tabTotal2:0) + ')'"
      ...
      <XTable :pageShow="true" v-model:page-size="tabQueryParams2.pageSize" :toolId="props.tabToolId2?props.tabToolId2:'tabToolId2'"
        ...
        <template #edit-quantity="{ row }">
          <el-input-number
            :disabled="props.drawerBorrow?.title?.includes('详情') || props.drawerBorrow?.title?.includes('审核')"
            :min="0" maxLength="11" :max="(row.inOutQuantity||row.inOutQuantity==0) ? row.inOutQuantity : row.outQuantity" :precision="props.tabQuantity2?props.tabQuantity2:0"
            placeholder="请输入数量"
            style="width: 99%;" @change="calculatePrice(row)" :controls="false" v-model="row.quantity" />
        </template>
        <template #default-quantity="{ row }">
          {{ row.quantity }}
        </template>
    ...
    <el-tab-pane v-if="props.tabName3" label="其他金额" name="其他金额">
      ...
      <XTable :pageShow="true" v-model:page-size="tabQueryParams2.pageSize" :intervalCondition="intervalCondition"
        size="mini" :data="accountOrderOtherList" :row-config="{ keyField: 'id' }"
        :checkbox-config="{ checkMethod: tabCheckMethod }" :columnList="otherColumnList" ref="otherPriceTableRef"
        @checkbox-all="selectOtherAllChangeEvent" height="100%" class="ptable-content"
        @checkbox-change="selectOtherChangeEvent" border :disabled="props.allData?.disabled">
        <template #default-createTime="scope">
          <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d}') }}</span>
        </template>
        ...
        <template #default-operate="scope">
          <el-button v-if="!props.drawerBorrow?.title?.includes('详情') && !props.drawerBorrow?.title?.includes('审核')" link
            type="primary" @click="handleUpdateOther(scope.row)">修改</el-button>
          <el-button v-if="!props.drawerBorrow?.title?.includes('详情') && !props.drawerBorrow?.title?.includes('审核')" link
            type="primary" @click="handleDeleteOther(scope.row)">删除</el-button>
        </template>
      </XTable>
    </el-tab-pane>
      ...
      <el-row>
        ...
        <el-col :span="24">
          <el-form-item label="金额:" prop="price">
            <el-input-number style="width: 100%; text-align: left;" :precision="2" min="0" :controls="false"
...
defineExpose({ tabQueryParams, tabQueryParams2, doSave, resetInfo });
