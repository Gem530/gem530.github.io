<template>
  <el-tabs v-model="tabRadioTable" @tab-click="tabRadioTableHandle" type="border-card" class="ptable-tab">
    ...
    >
      ...
      >
        ...
        <template #edit-quantity="{ row }">
          <el-input-number
            :disabled="props.drawerBorrow?.title?.includes('详情') || props.drawerBorrow?.title?.includes('审核') || (props.drawerBorrow?.title?.includes('确认')&& props.deduction)||props.disabled"
            maxLength="11"
            :min="0"
        ...
        <template #edit-discountPrice="{ row }">
          <el-input-number
            :disabled="props.drawerBorrow?.title?.includes('详情') || props.drawerBorrow?.title?.includes('审核')||props.disabled"
            :min="0"
            maxLength="11"
            :precision="2"
            style="width: 99%;"
    ...
    <el-tab-pane v-if="props.tabName3" :label="props.tabName3&&props.tabName3==true?'其他金额':props.tabName3" 
    :name="props.tabName3&&props.tabName3==true?'其他金额':props.tabName3">
      <el-row>
        <el-button v-if="otherAmountBtn" type="primary" @click="otherMoneyHandle"> 新增金额 </el-button>
        <el-button v-if="props.deduction && (!props.drawerBorrow?.title?.includes('详情'))"
          type="primary" @click="relatedDeduction" >添加扣款单</el-button
        >
      </el-row>
      <XTable
        height="400" 
        :pageShow="false" :scroll-y="{ enabled: true, gt: 20 }" 
        :intervalCondition="intervalCondition"
        size="mini"
        :data="accountOrderOtherList"
        :row-config="{ keyField: 'id' }"
        :checkbox-config="{ checkMethod: tabCheckMethodOther }"
        :columnList="props.tabColumnList3&&props.tabColumnList3.length>0?props.tabColumnList3:otherColumnList"
        ref="otherPriceTableRef"
        @checkbox-all="selectOtherAllChangeEvent"
        @checkbox-change="selectOtherChangeEvent"
        border
        :disabled="props.allData?.disabled"
        :toolId="props.tabToolId3?props.tabToolId3:'tabToolId3'"
        :showRefresh="true"
        @searchChange="tabSearchChange3"
      >
        <template #default-createTime="scope">
          <span>{{ parseTime(scope.row.createTime, '{y}-{m}-{d}') }}</span>
        </template>
        ...
        <template #default-operate="scope">
          <el-button
            v-if="!props.isSupplier&&(!props.drawerBorrow?.title?.includes('详情') && !props.drawerBorrow?.title?.includes('审核')) && ((scope.row.accountOrderDeductionId && props.deduction)||(!scope.row.accountOrderDeductionId && !props.deduction)||props.otherAmountBtn)"
            link
            type="primary"
    ...
    <el-tab-pane v-if="props.tabName4"  :label="props.tabName4" :name="props.tabName4">
      <XTable 
        height="400" 
          :pageShow="false" :scroll-y="{ enabled: true, gt: 20 }" 
          v-model:page-size="tabQueryParams4.pageSize"
          :intervalCondition="intervalCondition"
          v-model:current-page="tabQueryParams4.pageNum"
          :page-params="{ perfect: true, total: props.allData?.tabTotal4 }"
          size="mini" :data="props.allData?.tabList4"
          :row-config="{ keyField: 'id' }"
          :columnList="props.tabColumnList4&&props.tabColumnList4.length>0?props.tabColumnList4:adjustColumnList" ref="adjustTableRef"
          @searchChange="tabSearchChange4"
          border :disabled="props.allData?.disabled"
          :column-config="{ resizable: true }"
          :toolId="props.tabToolId4?props.tabToolId4:'tabToolId4'"
          :showRefresh="true" 
        >
          <template #default-type="scope">
            <span v-if="props.tabName4=='调整记录'">{{ scope.row.type == '1' ? '送货' : '退货' }}</span>
            <span v-else>{{ scope.row.type == '1' ? '单价' : (scope.row.type == '2' ?'数量':'折扣金额') }}</span>
          </template>
          <template #default-orderType="scope">
            <span>{{ scope.row.orderType == '1' ? '送货' : '退货' }}</span>
          </template>
          <template #default-isPrice="scope">
            <span>{{ scope.row.isPrice == '1' ? '单价' : '数量' }}</span>
          </template>
        </XTable>
    </el-tab-pane>
  </el-tabs>

  <el-form border label-width="120px" class="drawer-order-form" style="width: 100%;">
    <el-row>
      <el-col :span="6">
        <el-form-item
          size="small"
      ...
      <el-col :span="6" v-if="!props.footAdd">
        <el-form-item
          size="small"
      ...
      <el-col :span="6">
        <el-form-item
          size="small"
          :label="props.footLabel && props.footLabel?.label4 ? props.footLabel?.label4 + ':' : '扣款合计（元）：'"
  ...
  <el-dialog v-model="dialogChangeAddressAll.visible" :title="dialogChangeAddressAll.title" width="60%" destroy-on-close draggable>
    ...
    <div v-if="props.deductionSupplierName">当前供应商：<span style="color:#5d7db3">{{ props.deductionSupplierName }}</span>&nbsp;&nbsp;&nbsp;&nbsp;
      已选择
      <span style="color:#5d7db3">{{ HandleDetailList.length }}</span>
      条，共 <span style="color:#5d7db3">{{ HandleDetailList.reduce((acc, cur) => {
        return acc.plus(BigNumber(cur.price));
      }, BigNumber(0))}} 
      </span>
      元</div>
    <XTable
...
const intervalCondition = ['accountMonth', 'endTime', 'accountTime', 'createTime','operateTime','detailQuantity','inOutQuantity','backTime','deliveryTime'];
// 解决切换分页时，重置取消的勾选
...
const tabQueryParams4 = ref({
  pageNum: 1,
  pageSize: 20,
});
const tabQueryParams3 = ref({
  pageNum: 1,
  pageSize: 20,
});
...
//注入刷新方法.保存方法
const emits = defineEmits(['refreshFunction','refreshFunction1','refreshFunction2','refreshFunction4', 'doSaveBack']);
...
const adjustColumnList = ref([
  { title: "序号", fixed: 'left', type: 'seq', align: 'center', width: '60' },
  { title: '单据类型', width: '80', field: 'type', align: 'center',filterType:'radioButton',  filterData:()=>adjustTypeOption.value },
  ...
  { title: '调整项目',fixed: 'right', width: '80', field: 'isPrice', align: 'center',filterType:'radioButton',  filterData:()=>adjustPriceTypeOption.value},
  { title: '调整前', fixed: 'right',width: '80', field: 'adjustBefore', align: 'center' },
  { title: '调整后', fixed: 'right',width: '80', field: 'adjustAfter', align: 'center' },
  { title: '调整金额', fixed: 'right',width: '80', field: 'adjust', align: 'center' },
  { title: '调整时间', fixed: 'right',width: '140', field: 'createTime', align: 'center' },
]);
...
const tabSearchChange2 = (params: any) => {
  ...
  emits('refreshFunction2');
}
const tabSearchChange3 = (params: any) => {
  console.log("tabSearchChange3...",params)
  tabQueryParams3.value = params;
  if (params.sorts && params.sorts.length > 0) {
    params.sorts.forEach((item: any) => {
      tabQueryParams3.value.orderByColumn = item.prop;
      tabQueryParams3.value.isAsc = item.order;
    })
  }
  emits('refreshFunction3');
}

const tabSearchChange4 = (params: any) => {
  console.log("tabSearchChange4...",params)
  tabQueryParams4.value = params;
  if (params.sorts && params.sorts.length > 0) {
    params.sorts.forEach((item: any) => {
      tabQueryParams4.value.orderByColumn = item.prop;
      tabQueryParams4.value.isAsc = item.order;
    })
  }
  emits('refreshFunction4');
}
...
const tabCheckMethod = (row: any) => {
  return props.drawerBorrow?.title?.includes('新增');
}
// 切换菜单栏 *********** end
const tabCheckMethodOther = (row: any) => {

  return  props.drawerBorrow?.title?.includes('新增');
  //(!(props.drawerBorrow?.title?.includes('详情') || props.drawerBorrow?.title?.includes('审核') )) && ((row.row.accountOrderDeductionId && props.deduction)||((!row.row.accountOrderDeductionId && !props.deduction))|| props.otherAmountBtn) ;
}
...
let supplier =  useUserStore().supplier;
  ...
  accountOrderDeductionList.value = accountOrderDeductionList.value.map(item => {
    item.price=item.status=='3'&&item.closedPrice?item.closedPrice:item.price
    return item;
  });
...
const calculatePrice = async (row: any) => {
  ...
  //刷新底部合计
  tabRef1.value.xTableRef.updateFooter();
  tabRef2.value.xTableRef.updateFooter();
};
// 存储修改的数据
...
const resetInfo = () => {
  ...
  //不合计
  const excludedFields = ['saleOrderNum'];
  const footerMethod: VxeTablePropTypes.FooterMethod<any> = ({columns, data}) => {
    ...
    return [
      columns.map((column, columnIndex) => {
        ...
        if(column.field&&(column.field.includes("uantity") ||column.field.includes("totalPrice")||column.field.includes("discountPrice"))
        && !excludedFields.includes(column.field)){
  ...
  const sumNum = (list:any, field:any,sumField:any) => {
    ...
    let hasColon = false;
    list.forEach(item => {
        count = BigNumber(Number(item[field])).plus(BigNumber(Number(count))).dp(2).toNumber();
    });
...
onMounted(() => {
  ...
  console.log("props.isSupplier", props.isSupplier);
});

defineExpose({ tabQueryParams, tabQueryParams2,tabQueryParams3,tabQueryParams4, doSave, resetInfo, deductionList,doConfirm });
</script>
