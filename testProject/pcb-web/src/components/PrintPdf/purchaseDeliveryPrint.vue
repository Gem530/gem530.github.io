  ...
  <div v-if="deliverInfo" style="display:none; width: 600pt !important;padding-right:5pt;" ref="cardRef">
    <div style="width:100%;text-align:center;font-size:10pt;">
      ...
      <div style="width: 100%;display: flex;justify-content: center;">
        ...
        <div style="width: 60%;">
          <div style="font-size:18pt;font-weight:bold;">{{ deliverInfo.companyName }}</div>
      ...
      <div style="">
        工厂地址：{{ deliverInfo.companyAddress }}&nbsp;&nbsp;电话：{{ deliverInfo.companyPhone }}
      </div>
      ...
      <div style="display: flex; justify-content: space-between; padding: 2pt 0pt;">
        ...
        <div v-if="deliverInfo.deliverTemplates && deliverInfo.deliverTemplates.length > 0 &&deliverInfo.showType">
          客户名称：{{  deliverInfo.company }}</div>
        <div v-else style="">客户名称：{{ deliverInfo.company }}</div>
        <div style="width:170pt;text-align:left;">出货单号：{{ deliverInfo.code }}</div>
      </div>
      <div style="display: flex; justify-content: space-between; padding: 2pt 0pt;">
        <div style="text-align:left;width:400pt;">客户地址：{{ deliverInfo.deliveryAddress }}</div>
      ...
      <table style="border-collapse: collapse; border: none;width:100%;"
        ...
        <thead>
          <tr style="border: solid #000 1px; font-weight: normal; font-size: 9pt;">
            <th style="border: solid #000 1px;font-weight: normal;width: 90pt;">客户采购单号</th>
            <th style="border: solid #000 1px;font-weight: normal;width: 50pt;">物料编码</th>
            <th style="border: solid #000 1px;font-weight: normal;width: 50pt;">物料名称</th>
            <th style="border: solid #000 1px;font-weight: normal;width: 80pt;">规格参数</th>
            <th style="border: solid #000 1px;font-weight: normal;width: 25pt;">单位</th>
            <th style="border: solid #000 1px;font-weight: normal;width: 80pt;  ">要求交期</th>
            <th style="border: solid #000 1px; font-weight: normal; width: 38pt;">采购数量</th>
            <th style="border: solid #000 1px; font-weight: normal; width: 38pt;">送货数量</th>
            <th style="border: solid #000 1px; font-weight: normal; width: 20pt;">备品</th>
            <th style="border: solid #000 1px; font-weight: normal; width: 38pt;">差欠数量</th>
            <th style="border: solid #000 1px; font-weight: normal; width: 20pt;">箱数</th>
            <th style="border: solid #000 1px;font-weight: normal; width: 40pt;">备注</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="(item, index) in deliverInfo.purchaseMaterialDeliveryList">
            <td style="border: solid #000 1px; word-break: break-all; font-size: 9pt;padding:0px; text-align: center">
              {{ item.purchaseCode }}</td>
            <td style="border: solid #000 1px; word-break: break-all; font-size: 9pt;padding:0px; text-align: center">
            ...
            <td style="border: solid #000 1px; word-break: break-all; font-size: 6pt;padding:0px; text-align: center">
              <span style="font-size:10px;">{{ item.units }}</span>
            </td>
            <td style="border: solid #000 1px; word-break: break-all; font-size: 9pt;padding:0px;text-align: center;">
            ...
            <td style="border: solid #000 1px; word-break: break-all; font-size: 9pt;padding:0px;text-align: center;">
              {{ item.quantity }}</td>
            <td style="border: solid #000 1px; word-break: break-all; font-size: 9pt;padding:0px;text-align: center;">
              <span v-if="item.quantity">{{ item.deliveryQuantity }}</span>
            </td>
            <td style="border: solid #000 1px; word-break: break-all; font-size: 9pt;padding:0px;text-align: center;">
              {{ item.sparePartsQuantity }}
            </td>
            <td style="border: solid #000 1px; word-break: break-all; font-size: 9pt;padding:0px;text-align: center;">
              <span v-if="item.oweQuantity >= 0">{{ item.oweQuantity }}</span>
            </td>
            <td style="border: solid #000 1px; word-break: break-all; font-size: 9pt;padding:0px;text-align: center;">
              <span v-if="item.boxCount != 0">{{ item.boxCount }}</span>
            </td>
...
import {  getPurchaseDeliveryPrintDeliveryRecord } from "@/api/order/deliveryRecord";
...
const doPrint_back = async (_ids?: any) => {
  ...
  let dev = await getPurchaseDeliveryPrintDeliveryRecord(_ids as string);
...
const setQrCode = async ()=>{
  //二维码
  let _productionCardId: string = deliverInfo.value.id;
