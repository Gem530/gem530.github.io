<template>
  <div class="">
    ...
    <MIPrintPdf ref="cardRef" :allData="allData" :scrapped="scrapped" :printType="printType" :drillInfos="drillInfos"
      ...
      :drillHeaders="drillHeaders" :n_drillTitles="n_drillTitles" :n_drillHeaders="n_drillHeaders"
      :currentPdfList="currentPdfList" :commodityInfos="commodityInfos" :productionDetails="productionDetails"
      :publicCommodityInfo="publicCommodityInfo" v-if="allData && qcCodeImage && startPrint"></MIPrintPdf>
  </div>
</template>
...
const buildPublicCommodityInfo = async (result: any) => {
  if (Array.isArray(result.commodityInfos) && result.commodityInfos.length != 0) {
    ...
    console.log('_materialCopperInside', _materialCopperOutside);
    ...
    //测试方式拼接
    let _commodityTestWay:any='';
    result.commodityInfos.forEach((rc:any)=>{
      let has=_commodityTestWay.includes(rc.commodityTestWay);
      if(rc.commodityTestWay&&!has){
        _commodityTestWay=rc.commodityTestWay+"  "+_commodityTestWay;
      }
    })
    publicCommodityInfo.value.commodityTestWay =_commodityTestWay ;
  }
}

//ref.value.doPrint(data, type)
const doPrint = async (planId: any, type: string) => {
  ...
  //如果没有plan数量置空
  allData.value.pnlInfos.forEach((item: any) => {
    if (item.feedQuantity == "null"||item.feedQuantity == null || item.feedQuantity == undefined) {
      item.feedQuantity = '';
    }
  });

  let pnlPairs =JSON.parse(JSON.stringify(allData.value.pnlInfos));
  let newPnlList=[];
  for (let i = 0; i < pnlPairs.length; i += 2) {
    newPnlList.push(pnlPairs.slice(i, i + 2));
  }
  allData.value.newPnlList = newPnlList;
