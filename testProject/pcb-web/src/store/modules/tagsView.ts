...
export const useTagsViewStore = defineStore('tagsView', () => {
  ...
  const collectViews = useStorage('collectViews', [] as TagView[]);
  const refreshSale = ref(false)

  const setRefreshSale = (flag: boolean) => {
    refreshSale.value = flag
  }
  const addCollectViews = (view: TagView) => {
    if (collectViews.value.some((v) => v.path === view.path)) return;
    collectViews.value.push(
      Object.assign({}, view, {
        title: view.meta?.title || 'no-name'
      })
    );
  }
  const delCollectView = (view: TagView): Promise<TagView[]> => {
    return new Promise((resolve) => {
      for (const [i, v] of collectViews.value.entries()) {
        if (v.path === view.path) {
          collectViews.value.splice(i, 1);
          break;
        }
      }
      resolve([...collectViews.value]);
    });
  };
