...
export const useAppStore = defineStore('app', () => {
  ...
  const size = useStorage('size', 'small');
  const bold = useStorage('bold', 'default');
  ...
  const setBold = (b: string): void => {
    bold.value = b;
    if (bold.value == 'bold') {
      (document.body as any).style = '--el-vxe-table-weight: 600;'
    } else if (bold.value == 'default') {
      (document.body as any).style = '--el-vxe-table-weight: 400;'
    }
  };
