<template>
  <div class="p-2 xtable-page">
    <el-card shadow="never" class="xtable-card" v-if="checkPermi(['quality:AccountOrderCusDeduction:list'])">
      <template #header>
        <el-row :gutter="10" class="mb8 global-flex flex-end">
          <el-col :span="1.5">
            <el-button v-hasPermi="['quality:AccountOrderCusDeduction:add']" type="primary" @click="handleAdd">新增客户扣款</el-button>
            <el-button v-hasPermi="['quality:AccountOrderCusDeduction:export']" type="primary" @click="handleExport">导出</el-button>
          </el-col>
          <!-- <el-col :span="1.5">
      ...
      <XTable
        ...
        toolId="financialAccountOrderCusDeduction"
      ...
      >
        ...
        <template #default-make="scope">
          ...
          <el-button v-hasPermi="['quality:AccountOrderCusDeduction:edit']" link v-if="!scope.row.hasAccountOrder && ['1','4'].includes(String(scope.row.status))" type="primary" @click="handleUpdate(scope.row)">{{'编辑'}}</el-button>
          <el-button v-hasPermi="['quality:AccountOrderCusDeduction:audit']" link v-if="!scope.row.hasAccountOrder && scope.row.status == '2' " type="primary" @click="handleUpdate(scope.row)">{{'审核'}}</el-button>
          <el-button v-hasPermi="['quality:AccountOrderCusDeduction:reAudit']" link v-if="!scope.row.hasAccountOrder && scope.row.status == '3'" type="primary" @click="handleReverseAudit(scope.row)">{{'反审核'}}</el-button>
          <el-button v-hasPermi="['quality:AccountOrderCusDeduction:delete']" link v-if="!scope.row.hasAccountOrder && ['1','4'].includes(String(scope.row.status))" type="primary" @click="handleDelete(scope.row)">删除</el-button>
        </template>
      </XTable>
    </el-card>
    ...
    >
      <el-form :model="form"  ref="AccountOrderDeductionFormRef" label-width="100px"  :rules="rules">
        ...
        <el-input v-model="form.checkRemark" type="textarea" :rows="4" show-word-limit maxlength="200" minlength="1"
            ...
            <el-row>
              ...
              <el-col :span="12">
                <el-form-item label="选择客户：" prop="customerId">
                  ...
                  >
                    <el-option v-for="item in customerList" :key="item.value" :label="item.customerCode" :value="item.value">
...
<!--                      <span style="margin-left:20px;">{{ item.label }}</span>-->
          ...
          <el-collapse-item title="责任分配" name="2">
            <el-form class="account-date-width" :model="form" :rules="rules" label-width="120px" :disabled="dialog.title?.includes('详情')">
              <el-row>
        ...
        <el-collapse-item title="凭证信息" name="3">
          ...
          <FileTable :data="form.sysFileList" makeWidth="160">
            <template #makeBtn="{row}">
              <el-button v-if="dialog.title != '客户扣款详情'" link type="primary" @click="delFile(row)">删除</el-button>
            </template>
          </FileTable>
        </el-collapse-item>
      ...
      <template #footer>
        <div class="global-flex flex-center" v-loading="dialogLoading">
          <template v-if="!viewStatus && (dialog.title == '新增客户扣款' || ['2','3'].includes(String(form.status)))">
...
<!--          </template>-->
          ...
          <template v-if="!viewStatus && ( dialog.title == '新增客户扣款' || ['1','4'].includes(String(form.status)))">
...
<script setup name="AccountOrderCusDeduction" lang="ts">
...
const columnList = ref([
  ...
  { title: '客户代码', width: '160', field: 'ownerCode', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入客户代码' } },
...
/** 反审核按钮操作 */
const handleReverseAudit = async (row?: any) => {
  // 如果有责任金额则该单只能在当月反审核
  if(row.dutyPrice) {
    // 创建时间不能大于当月
    let nowDate = new Date();
    let createDate = new Date(row.createTime);
    if (!(nowDate.getFullYear() == createDate.getFullYear() && nowDate.getMonth() == createDate.getMonth())) {
      ElMessage.error("该扣款单只能在创建当月修改");
      return;
    }
  }
  handleUpdate(row);
        ...
        if (params.type == 1) {
          ...
          if (responsibilityData.value?.length) {
            ...
            if (params.deductionDutyList.length > 1) {
              ...
              params.deductionDutyList.forEach((item : any) => { tempDutyPrice = tempDutyPrice + item.price})
            } else {
              tempDutyPrice = params.deductionDutyList[0].price
            }
            console.log(tempDutyPrice)
            console.log(params.price)
            if ( !params.closedPrice && Number(tempDutyPrice.toFixed(2)) > params.price ) {
              proxy?.$modal.msgError('责任金额之和不能大于发出金额')
              return
            }
            if (params.closedPrice && tempDutyPrice > params.closedPrice ) {
              proxy?.$modal.msgError('责任金额之和不能大于结案金额')
              return
            }
          }
        } else {
...
const handleExport = () => {
  if (
    ...
    queryParams.value.ownerCode === undefined &&
    ...
    queryParams.value.dutyName   === undefined &&
...
const exportList = () =>{
  proxy?.download('financial/AccountOrderDeduction/export', {
    ...queryParams.value, tableName: 'financialAccountOrderCusDeduction'
  }, `客户扣款记录_${formatDate(new Date())}.xlsx`);
}
