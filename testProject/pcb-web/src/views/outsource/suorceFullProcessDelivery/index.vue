        ...
        <el-tab-pane label="待发货列表"      name="0">
          ...
          >
            ...
            <template #default-units="scope">
                <el-tag type="info" size="small" >PCS</el-tag>
            </template>
          ...
          <el-col :span="24" style="margin-top: 20px">
            ...
            >
              ...
              <template #default-deliveryQuantity="scope" >
                <el-input-number  style="width: 99%;" :precision="0"   :controls="false"  v-model="scope.row.deliveryQuantity" :min="1" :max="scope.row.canDeliveryQuantity ? scope.row.canDeliveryQuantity :scope.row.oweQuantity"/>
              </template>

              <template #default-sparePartsQuantity="scope">
                <el-input-number  :disabled="isEdit" :precision="0"  style="width: 99%;"  :controls="false"  v-model="scope.row.sparePartsQuantity" :min="1"  :max="999999999"/>
              </template>
              <template #default-boxCount="scope">
                <el-input-number  :disabled="isEdit" :precision="0"  style="width: 99%;"  :controls="false"  v-model="scope.row.boxCount" :min="1"  :max="99999999"/>
              </template>
              <template #default-remark="scope">
                <el-input  :disabled="isEdit" style="width: 99%;"  :controls="false"  v-model="scope.row.remark"  maxlength="100"/>
              </template>
              <template #default-make="scope">
...
import {
  ...
  viewDeliveryDetail,
  delDelivery
} from "@/api/outsource/suorceFullProcessDelivery";
import {ref} from "vue";
  ...
  if(row.status==="3"){
    getSignPdf({bizId:row.bizId,bizType:'19'}).then(res=>{
      ...
      if (vo !== null && vo.url) {
        ...
        reportUrl.value = getReportUrl() + `&_n=外协送货单&_u=file:outSourceFullDelivery.ureport.xml&url=outsource/sourceFullProcessDelivery/reportHeard/${row.id}&listUrl=outsource/sourceFullProcessDelivery/reportExcel/${row.id}`;
      }
    });
    return;
  }
...
// 声名需要处理的时间查询条件字段
const intervalCondition = ['updateTime','createTime','deliverTime','deliveryTime','deliveryTimeStr'];
// 初始化可生成送货单页面查询参数
...
const purchaseOrderColumnList = ref([
  ...
  { title: '外协单号', field: 'code', width: '125', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入外协单号' } },
  { title: '供应商名称', field: 'supplierName', width: '160', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入供应商名称' } },
  { title: '客户名称', field: 'company', width: '160', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入客户名称' } },
  ...
  { title: '产品名称', field: 'commodityName', width: '110', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入物料名称' } },
  { title: '单位名称', field: 'units', width: '80', align: 'center', filterParam: { placeholder: '请输入单位名称' }, },
  { title: '订单数量' , sortable: true, field: 'quantity', width: '80', align: 'center' },
  { title: '已发数量', field: 'deliverQuantity', width: '90', align: 'center', filterParam: { placeholder: '' }},
...
const confirmSelectInventoryColumnList = ref([
  ...
  { title: '外协单号', field: 'outCode', width: '125', align: 'center',fixed: 'left' , showOverflow: false },
  { title: '产品编码', field: 'commodityCode', width: '110', align: 'center',fixed: 'left' , showOverflow: false },
  { title: '产品名称', field: 'commodityName', width: '110', align: 'center'},
  // { title: '供应商名称', field: 'supplierName', width: '160', align: 'center' },
  { title: '客户名称', field: 'company', width: '160', align: 'center'},
  { title: "要求交期"  , sortable: true,  width: "140", field: "deliverTime", align: "center"},
  { title: '单位名称', field: 'units', width: '80', align: 'center' },
  { title: '订单数量' , field: 'quantity', width: '80', align: 'center'},
...
const resetReceiveForm = () => {
  ...
  receiveFromData = ref({
    ...
    outSourceFullProcessDeliveryList: [],
    status: '',
    remark: ''
  });
  confirmSelectInventoryList.value = []
...
const editDeliveryOrder = async () => {
    ...
    if (isEdit.value === true) {
      ...
      // 确认
      receiveFromData.value.outSourceFullProcessDeliveryList = confirmSelectInventoryList.value;
...
const deliveryWaitConfirmColumnList = ref([
  ...
  { title: '外协单号', field: 'outCode', width: '116', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入外协单号' } },
  { title: "要求交期"  , width: "140", field: "deliveryTimeStr", align: "center", filterType: 'intervalDate',
    filterParam: {
      startParams: {placeholder: '请输入开始时间', type: 'datetime', valueFormat: 'YYYY-MM-DD'},
      endParams: {placeholder: '请输入结束时间', type: 'datetime', valueFormat: 'YYYY-MM-DD'},
    }},
  { title: '产品编码', field: 'commodityCode', width: '110', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入物料编码' } },
  { title: '产品名称', field: 'commodityName', width: '110', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入物料名称' } },
  { title: '单位', field: 'units', width: '50', align: 'center' },
  ...
  { title: '操作',  align: "center", field: "make" ,fixed: 'right' , width: '330', showOverflow: false  }
]);
...
const mergeCells = ({ row, items, seq, rowIndex, $rowIndex, _rowIndex, column, data }) => {
  ...
  } else if (column.property === "deliveryTimeStr") {
    // 修改createTime的合并逻辑，使其依赖于code的值
    const referenceField = "deliveryTimeStr";
    const codeField = "code";
    if (rowIndex > 0 && row[codeField] === items[rowIndex - 1][codeField]) {
      if (row[referenceField] === items[rowIndex - 1][referenceField]) {
        return { rowspan: 0, colspan: 0 };
  ...
  if (deliveryWaitConfirmTableData.value === null){
    ElMessageBox.confirm(
      ...
      '导出提示:',
      {
        ...
        proxy?.download("/outsource/sourceFullProcessDelivery/export", {
...
const viewDelivery = async (row) => {
  ...
  viewDeliveryDetail(row.id).then((res) => {
    ...
    confirmSelectInventoryList.value = res.data.outSourceFullProcessDeliveryList;
  }).finally(() => {
