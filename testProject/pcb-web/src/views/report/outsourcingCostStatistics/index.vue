<template>
  <div class="p-2 xtable-page">
    <transition :enter-active-class="proxy?.animate.searchAnimate.enter"
      ...
      <div class="search">
        <el-form :model="queryParams" ref="queryFormRef" label-width="85px"
                 class="demo-form-inline">
          <el-row>
            <el-col :span="8">
              <el-form-item label="所属月份" prop="timeBetween">
                <IntervalDate
                  :auto="true"
                  :linkPanels="false"
                  :isAcceptValue="true"
                  v-model="queryParams.timeBetween"
                  :endParams="{type: 'month',placeholder: '选择开始时间',valueFormat:'YYYY-MM',disabledDate: dateDisable2}"
                  :startParams="{type: 'month',placeholder: '选择开始时间',valueFormat:'YYYY-MM',disabledDate: dateDisable1}"
                ></IntervalDate>
              </el-form-item>
            </el-col>
            <el-col :span="12">
            ...
            <el-col :span="4">
              <div class="width-100 global-flex flex-end">
                <el-button type="primary" :disabled="loading" @click="exportData" v-hasPermi="['report:outsourcingCostStatistics:export']">导出</el-button>
              </div>
            </el-col>
          </el-row>
        </el-form>
    ...
    <el-card shadow="never" class="xtable-card">
      ...
      <XTable
        ...
        <template #default-costMonth="{ row }">
          {{ dayjs(row.costMonth).format('YYYY-MM') }}
        </template>
...
<script setup name="OutsourcingCostStatistics" lang="ts">
import dayjs from 'dayjs'
import { deepClone } from '@/utils';
import { getCraftListOutsourcCost } from '@/api/report/outsourcingCostStatistics/index';
...
const initColumn = [
  ...
  // filterType: 'date', filterParam: { type: 'month', placeholder: '请选择所属月份',valueFormat: 'YYYY-MM' } 
  },
  { title: '合计', field: 'totalCost', align: 'center', fixed: 'right', width: '120', sortable: true },
...
const dateDisable1 = (date: Date) => {
  let date2 = queryParams.value.timeBetween[1]
  date2 = date2 ? date2+'-01' : date2
  if (!date2) return false

  // 不能大于结束时间
  let dis1 = date.getTime() > new Date(dayjs(date2).format('YYYY-MM-DD')).getTime()
  // 不能小于结束时间一年
  let dis2 = date.getTime() < new Date(dayjs(date2).subtract(1,'year').format('YYYY-MM-DD')).getTime()
  return dis1 || dis2
}

const dateDisable2 = (date: Date) => {
  let date1 = queryParams.value.timeBetween[0]
  date1 = date1 ? date1+'-01' : date1
  if (!date1) return false

  // 不能少于开始时间
  let dis1 = date.getTime() < new Date(dayjs(date1).format('YYYY-MM-DD')).getTime()
  // 不能大于开始时间一年
  let dis2 = date.getTime() > new Date(dayjs(date1).add(1,'year').format('YYYY-MM-DD')).getTime()
  return dis1 || dis2
}
/** 查询列表 */
const getList = async () => {
  ...
  let res: any = await getCraftListOutsourcCost(params);
  // let tempTotal = 10100
  // let tempRows: any = []
  // for (let i = 0; i < tempTotal; i++) {
  //   tempRows.push({test: Math.random()})
  // }
  // res = {
  //   code: 200,
  //   rows: tempRows,
  ...
  // if (res?.rows?.length) {
    ...
    // })
    if (res.data?.length) {
      columnList.value = JSON.parse(JSON.stringify(initColumn))
      let tempColumn: any[] = []
      let level = ['one', 'two', 'thr', 'fur', 'fiv', 'six', 'sev']
      if (res.data[0].typeCraftList?.length) {
        res.data[0].typeCraftList.map((subList: any[], subI: number) => {
          let tempSub: any = {
            craftName: '',
            list: []
          }
          subList.map((m: any, mi: number) => {
            if (m?.type == 3) {
              tempColumn.push({
                title: m.craftName,
                field: `typeCraftList[${subI}][${mi}].price`,
                align: 'center',
                minWidth: '80',
                sortable: true
              })
            } else {
              tempSub.craftName = m.typeName
              tempSub.list.push({
                title: m.craftName,
                minWidth: '80',
                field: `typeCraftList[${subI}][${mi}].price`,
          ...
          if (tempSub?.list?.length) {
            console.log(tempSub)
            tempColumn.push({
              title: tempSub.craftName,
              width: '80',
              field: level[subI],
              align: 'center',
              children: tempSub.list
            })
          }
        })
      }
      columnList.value.splice(3, 0, ...(tempColumn || []))
    }
    //  else {
