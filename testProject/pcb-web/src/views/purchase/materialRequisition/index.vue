  ...
  <div class="p-2 xtable-page">
      ...
      <el-tabs v-model="editableTabsValue" @tab-change="getVoidedList()" class="xtable-tab">
        <el-tab-pane label="按领料单展示" :name="1">
          <div class="head-btn-flex start">
            <el-radio-group v-model="radioTable" size="small">
              <el-radio-button v-if="checkPermi(['purchase:materialRequisition:audit'])" :label="2" @change="listRadio('2')"
                >待审核列表
              </el-radio-button>
          ...
          >
            <template #content-expand="{ row }">
              ...
              >
                ...
                <template #default-make="scope">
                  <el-button link type="primary" @click="getHistoryList(scope.row)">审核记录</el-button>
                  <el-button v-if="scope.row.status =='3' && scope.row.returnableQuantity>0" link type="primary" @click="requisitionReturn(scope.row)"
                    >还料
                  </el-button>
                  <el-button
                    v-if="scope.row.status =='3' && scope.row.returnableQuantity<scope.row.quantity"
                    link
                    type="primary"
                    @click="requisitionReturnRecord(scope.row)"
                    >还料记录
                  </el-button>
                </template>
              </XTable>
            ...
            <template #default-make="scope">
              <el-button v-if="scope.row.status =='1'" link type="primary" @click="handleUpdate(scope.row)">修改 </el-button>
              <el-button v-if="scope.row.status =='1'" link type="primary" @click="submitData(scope.row)">提交 </el-button>
              <el-button v-if="scope.row.status =='1'" link type="primary" @click="deleteData(scope.row)">删除 </el-button>
              <el-button
                v-if="(scope.row.status =='2' || scope.row.status =='4') && radioTable =='2' && checkPermi(['purchase:materialRequisition:audit'])"
                link
                type="primary"
        ...
        <el-tab-pane label="按物料展示" :name="2">
          ...
          <XTable
            height="100%"
            class="xtable-content"
            toolId="purchaseMaterialRequisition"
    ...
    <!--    新增物料领料申请抽屉-->
    <el-drawer :title="dialog.title" v-model="dialog.visible" size="80%" destroy-on-close @close="cancelCommit">
      <div class="ptable-card">
        ...
        >
          <el-row>
            <el-col :span="6">
              <el-form-item label="领料部门" prop="createDept">
                <el-select
                  :disabled="insertStatus"
                  style="width: 100%;"
                  v-model="form.createDept"
                  @change="handleDetSelect"
                  filterable
                  placeholder="请选择"
                >
                  <el-option v-for="item in deptVOs" :key="item.deptId" :label="item.deptName" :value="item.deptId"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6">
              <el-form-item label="领料人" prop="createBy">
                <el-select
                  :disabled="insertStatus"
                  style="width: 100%;"
                  ...
                  placeholder="请选择"
                >
                  <el-option v-for="item in userVOs" :key="item.userId" :label="item.nickName" :value="item.userId"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6" v-if="haveCraft && (dialog.title != '物料申请详情' && dialog.title != '修改物料申请')">
              <el-form-item label="选择工序" prop="craftId">
                <el-select
                  :disabled="updateStatus"
                  style="width: 100%;"
                  v-model="form.craftId"
                  @change="refreshAndGetMaterialInventory"
                  filterable
                  placeholder="请选择"
                >
                  <el-option v-for="item in crafts" :key="item.id" :label="item.craftName" :value="item.id"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6" v-if="haveCraft && (dialog.title == '物料申请详情' || dialog.title == '修改物料申请')">
              <el-form-item label="选择工序" prop="craftId">
                <el-select
                  ...
                  placeholder="请选择"
                >
                  <el-option v-for="item in allCrafts" :key="item.id" :label="item.craftName" :value="item.id"></el-option>
                </el-select>
              </el-form-item>
            </el-col>
            <el-col :span="6">
        ...
        <!--      </el-row>-->
        <el-divider content-position="left" style=" margin-top: 10px; margin-bottom: 0px;">已选物料</el-divider>
        ...
        >
          ...
          <template #default-quantity="scope">
            <el-input-number
              ...
              :step="0"
              :controls="false"
              :max="Number(scope.row.actualStock)"
              :precision="2"
      ...
      <template #footer>
        <el-button v-if="dialog.title != '物料申请详情'" :loading="buttonLoading" type="primary" @click="submitForm('1')">保 存</el-button>
        <el-button v-if="dialog.title != '物料申请详情'" :loading="buttonLoading" type="primary" @click="submitForm('2')">保存并提交</el-button>
        <el-button :loading="buttonLoading" @click="cancelCommit()">取消</el-button>
      </template>
    </el-drawer>
      ...
      <el-row>
        <el-col :span="5">
          <XTable
            ...
            :pageShow="false"
            :data="selectInventoryList"
            :columnList="selectedMaterialColumnListChoose"
    ...
    <el-dialog title="审批备注" v-model="dialogSubmit.visible" width="30%" destroy-on-close>
      <el-form :model="form" label-width="100px">
        <el-form-item label="审批备注">
          <el-input v-model="form.checkRemark" type="textarea" :rows="4" show-word-limit maxlength="500" placeholder="请输入"></el-input>
        </el-form-item>
      </el-form>
    ...
    <el-dialog
      ...
      v-loading="isLoading"
    >
      <XTable toolId="chooseScrapProduction" :page-show="false" :data="flowList" :columnList="flowColumnList" height="500px" :isAutoHeight="true">
    ...
    <RequisitionReturnDialog
      v-if="dialogReturn.visible"
      @refresh="getVoidedList"
      v-model:show="dialogReturn.visible"
      :title="dialogReturn.title"
      :formShow="formShow"
      :formData="returnForm"
    ></RequisitionReturnDialog>
    ...
    <div v-if="currentPrintInfo">
      <div style="display: none; width: 600pt !important;padding-right:5pt;" ref="cardRef">
        <div style="width:100%;text-align:center;font-size:10pt;">
          ...
          <div style="">
            工厂地址：{{
              currentPrintInfo.enterpriseAddress


            }}&nbsp;&nbsp;电话：{{ currentPrintInfo.enterprisePhoneNumber }}
          </div>
          <div style="font-size:16pt;font-weight:600;">领&nbsp;料&nbsp;单</div>
          <div style="padding-top: 10px">
            <span style="float: left">领料单号：{{ currentPrintInfo.code }}</span
            ><span style="float: right">领料部门：{{ currentPrintInfo.createDeptName }}</span>
          </div>
        </div>
...
const data = reactive<PageData<MaterialApplyForm, MaterialApplyQuery>>({
  ...
  rules: {
    ...
    createBy: [
      {required: true, message: "请选择领料人", trigger: "change"}
    ],
    createDept: [
      {required: true, message: "请选择领料部门", trigger: "change"}
    ],
  }
});
...
const materialColumnList = ref([
  ...
  {title: '使用工序', field: 'craftName', align: 'center', width: '80',},
  ...
  {title: '领用总数', field: 'quantity', align: 'center', width: '80',},
  {title: '还料数量', field: 'actuallyQuantity', align: 'center', width: '80',},
  {title: '实际领用数量', field: 'returnableQuantity', align: 'center', width: '80',},
...
const selectedMaterialColumnListChoose = ref([
  {title: '', field: 'rawMaterialCode', align: 'center', width: '70',},
  {title: '已选物料', field: 'name', align: 'center', minWidth: '70',},
  {title: '', field: 'make', align: 'center', width: '40',},
]);
...
const confirmSelectInventoryColumnList = ref([
  ...
  {title: '数量', field: 'quantity', fixed: 'right', width: '100', align: 'center',},
...
import {listUserByDeptId, userListByDeptId} from "@/api/system/user";
import {listDeptByOwnerId} from "@/api/system/dept";
...
const refreshAndGetMaterialInventory = async () => {
  confirmReset();
  await getMaterialInventory();
}
...
const getMaterialInventory = async () => {
  ...
  selectQueryParams.value.craftId = form.value.craftId;
...
const getAllDepts = async () => {
  const res = await listDeptByOwnerId(useUserStore().ownerId)
  console.log("查询领料部门", res)
  deptVOs.value = res.data;
}

const userVOs = ref([]);
/** 查询当前账号的部门对应的用户 **/
const handleDetSelect = async (deptId) => {
  form.value.createBy = "";
  form.value.craftId = "";
  console.log("查询领料部门", deptId)
  const res = await userListByDeptId(deptId)
  await getCraftList(deptId)
  console.log("查询领料部门----------------------", res.data)
  userVOs.value = res.data
  console.log("crafts",crafts.value)
}

// 更新状态
const insertStatus = ref(true)
...
const handleAdd = async () => {
  ...
  //判断当前人是否是特殊领料部门下面的人
  let params = {
    code: "MATERIAL_SPECIAL"
  }
  const res = await listTenantConfig(params);
  if (res && res[0].defaultValue.includes(useUserStore().deptId)) {
    insertStatus.value = false;
    await getAllDepts();
    //进来默认先取当前单位部门对应的用户列表
    await handleDetSelect(useUserStore().deptId)
    form.value.createBy = useUserStore().userId;
    form.value.createDept = useUserStore().deptId;
  }else {
    form.value.createBy = useUserStore().userId;
    form.value.createDept = useUserStore().deptId;
    userVOs.value.push({userId:form.value.createBy,nickName:useUserStore().nickname})
    deptVOs.value.push({deptId:form.value.createDept,deptName:useUserStore().deptName})
  }
  form.value.createTime = dayjs(new Date()).format("YYYY-MM-DD HH:mm:ss")
  updateStatus.value = false;
  haveCraft.value = true;
  dialog.visible = true;
  dialog.title = "物料申请";
  confirmReset();
...
const handleUpdate = async (row?: any) => {
  ...
  const tenant = await listTenantConfig(params);
  if (tenant && tenant[0].defaultValue.includes(useUserStore().deptId)) {
    insertStatus.value = true;
    updateStatus.value = true;
    await getAllDepts();
    //进来默认先取当前单位部门对应的用户列表
    await handleDetSelect(res.data[0].createDept)
  }else {
    updateStatus.value = true;
    userVOs.value.push({userId:res.data[0]?.createBy,nickName:res.data[0]?.createByName})
    deptVOs.value.push({deptId:res.data[0].createDept,deptName:res.data[0].createDeptName})
  }
  form.value.createBy = res.data[0]?.createBy;
  form.value.createDept = res.data[0].createDept;
  form.value.craftId = res.data[0]?.craftId;
  if (res.data[0]?.craftId == null) {
    haveCraft.value = false
  } else {
    haveCraft.value = true
  }
  dialog.visible = true;
...
const handleSelect = async (row?: MaterialApplyVO) => {
  ...
  form.value.createBy = res.data[0]?.createByName;
  form.value.createDept = res.data[0].createDeptName;
...
const deleteData = async (row: any) => {
  ...
  if (1 == editableTabsValue.value) {
    ...
    row.inventoryList = res.data;
    _ids = row.inventoryList.map((vo: any) => {
      return vo.id
    });
  } else {
    _ids.push(row.id)
...
const auditData = (row: any) => {
  const ids = selectRequisitionByMaterial.value?.map((vo: any) => {
    return vo.id
  });
...
const handlePass = async () => {
  // 查询是否存在盘点中物料
  let materialIds = selectRequisitionByMaterial.value?.map((vo: any) => {
    return vo.rawMaterialId
  });
  if (!materialIds || materialIds.length == 0) {
    const materiaRes = await getMaterialRequisitionsByCodeSelf(currentInfo.value.code);
    materialIds = materiaRes.data.map(item => item.rawMaterialId);
  }
  let query = {
    pageNum: 1,
    pageSize: 20,
  ...
  const res1 = await listMaterialInventoryInfo(query);
  if (res1.rows && res1.rows.length > 0) {
    inventoryRes.value = res1.rows;
...
const submitForm = (status: string) => {
  materialApplyFormRef.value?.validate(async (valid: boolean) => {
    if (form.value.craftId == undefined) {
      proxy?.$modal.msgError("请选择工序");
      return;
    }
    ...
    if (valid) {
      ...
      if (dialog.title.includes('修改')) {
        ...
        console.log("data:",deptVOs.value.find(v=> v.deptId == form.value.createDept))
        const data = {
          addList: form.value.addList,
          createByName: userVOs.value.find(v=> v.userId == form.value.createBy).nickName,
          createBy: form.value.createBy,
          createTime: form.value.createTime,
          createDeptName: deptVOs.value.find(v=> v.deptId == form.value.createDept).deptName,
          createDept: form.value.createDept,
        ...
        form.value.createByName = userVOs.value.find(v=> v.userId == form.value.createBy).nickName
        form.value.createDeptName = deptVOs.value.find(v=> v.deptId == form.value.createDept).deptName
...
const getHistoryList = async (row: any) => {
  ...
  // }
  //审核记录过滤多余数据
  if (flowList.value.length > 0 && !flowList.value[0].duration) {
    flowList.value.shift();
  }
...
// let crafts: CraftVO[] = [];
const crafts = ref([]);
const getCraftList = async (deptId) => {
  let isFilterDept = "1";
  // 本厂管理员不需要过滤
  // if (userId.value === 6389){
  //   isFilterDept = "0";
  // }
  if (!deptId){
    deptId = useUserStore().deptId
  }
  const res = await listCraft({isFilterDept: isFilterDept, isOpen: "1",deptId:deptId});
  console.log("res",res);
  crafts.value = res.rows;
  crafts.value?.forEach(item => {
 ...
 * 查询工艺列表
 */
let allCrafts: CraftVO[] = [];
const getAllCraftList = async () => {
  const res = await listCraft({isOpen: "1"});
  console.log(res);
  allCrafts = res.rows;
  allCrafts.forEach(item => {
    item.craftName = String(item.craftName);
  });
};
/**
...
watch(() => route.query?.pendingParams, (newVal) => {
  if (newVal) {
    ...
    if (decryptStr && decryptStr != '{}') {
      ...
      let radio = !isNaN(Number(params.radio)) ? params.radio : '0';
      editableTabsValue.value = tab
      radioTable.value = radio
      if (radio === '2') {
        let tempColumnList = [{field: 'code', defaultValue: params.bizNo}]
        queryParams.value.code = params.bizNo
        setTimeout(() => {
          pickingXTableRef.value.filterFieldEvent(tempColumnList)
        }, 100)
      }
    }
  }
