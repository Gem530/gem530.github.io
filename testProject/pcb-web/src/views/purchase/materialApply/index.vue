<template>
  <div class="p-2 xtable-page">
    <el-card shadow="never" class="xtable-card">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5" style="text-align: right;width: 100%">
    ...
    <el-drawer :title="dialog.title" v-model="dialog.visible" size="80%" :destroy-on-close="true">
      <el-form :model="form" label-width="80px" ref="materialApplyFormRef" :rules="rules" :disabled="dialog.title === '物料申请详情'" v-loading="dialogTableLoading">
        <el-row>
          <el-col :span="20">
            <el-form-item label="收货地址" prop="addressId">
              <el-select  style="width:100%;" v-model="form.addressId" placeholder=" " filterable>
                <el-option v-for="item in customerAddressList"
                :key="item.id"
...
<!--        </el-col>-->
        ...
        <el-main style="padding:0px;height: calc(100% - 110px);overflow-x:hidden;">
          <div style="flex: none;height: 40% !important;" class="ptable-card">
      <el-divider content-position="left">已选物料</el-divider>
      <XTable toolId="materialApplyDrawerTable" :pageShow="false" ref="confirmTable" :data="confirmSelectInventoryList" border :columnList="confirmSelectInventoryColumnList" class="ptable-content" height="100%">
        <!-- <template #default-categoryId="scope">
          <div>{{ scope.row.categoryName }}</div>
        </template> -->
        ...
        </template> :disabled-date="disabledDate"-->
        ...
        <template #default-quantity="scope">
          <el-input-number :disabled="dialog.title === '物料申请详情'" style="width: 99%;"  :step="0.1" :controls="false" :precision="2" v-model="scope.row.quantity" />
        </template>
        <template #default-applyRemark="scope">
          <el-input :disabled="dialog.title === '物料申请详情'" :rows="1" :max="200" type="textarea" v-model="scope.row.applyRemark" autocomplete="off" />
        </template>
        <template #default-make="scope">
          <el-button
    ...
    <div style="flex: none;height: 60% !important;" class="ptable-card">
      <el-divider v-if="dialog.title != '物料申请详情'"  content-position="left">待选物料</el-divider>
        <XTable v-if="dialog.title != '物料申请详情'"
...
import { listMaterialApply, getMaterialApply, delMaterialApply, addMaterialApply, updateMaterialApply, listMaterialInventory, modifyStatus,listApplyByMaterial } from '@/api/purchase/materialApply';
