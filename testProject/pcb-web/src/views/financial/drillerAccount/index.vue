<template>
  <div class="p-2 xtable-page">
    ...
    <accountTable ref="accountTableRef" :writeOffType="writeOffType" :tableColumnList="columnList"
      toolId1="purAccountToolId1" toolId2="purAccountToolId12" toolId3="purAccountToolId13" exportName="研磨对账单"
     :type="accountOrderType" @handle="handle" :writeOffIsSupplier="true" :titleLabel="titleLabel" />
    ...
    <el-drawer v-model="drawerBorrow.visible" :title="drawerBorrow.title" size="90%" draggable destroy-on-close
      ...
      <accountForm ref="accountFormRef" v-if="drawerBorrow.visible" :row="formInfo" showIsTax="true" @handleSearch="handleInOrOutQuery"
        :type="formType" :isEdit="isEdit" />

      <RawAccountTab tabToolId1="procureToolId1" tabToolId2="procureToolId2" ref="rawAccountTabRef" :allData="allData" :tabColumnList="tabColumnList"
        :editOtherList="tabOtherList" :tabName1="'研磨还回'" :tabName2="'研磨报废'" :tabName3="true" :drawerBorrow="drawerBorrow"
        :tabColumnList2="backTabColumnList" @getCheckFunction="false" @refreshFunction="deliverSearchChange" :footLabel="footLabel"
      ...
      <!-- </div> -->
        ...
        <div style="display: flex; justify-content: center;">
          <span class="dialog-footer">
            ...
            <el-button type="danger" v-show="drawerBorrow.title?.includes('审核')" @click="reject"> 驳回
            </el-button>
            <el-button type="primary" v-show="drawerBorrow.title?.includes('审核')" @click="examinePass">
              通过 </el-button>
          </span>
        </div>
      </template>
...
import {
  DrillerOperationTypeEnum,
} from '@/api/basedata/rawMaterial/types';
import { parseTime } from "@/utils/ruoyi";

const accountOrderType = TypeEnum.DRILLER;
...
const tabColumnList = ref([
  ...
  { title: '还回日期', sortable: true, width: '140', field: 'createTime', align: 'center' },
  { width: '120', title: '研磨单号', field: 'code', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入研磨单号' } },
  { width: '120', title: '供应商编码', field: 'supplierCode', align: 'center' },
...
const backTabColumnList = ref([
  { width: '40', type: 'checkbox', align: 'center', field: "checkbox", visible: true, },
  { width: '40', title: '序号', type: 'seq', align: 'center' },
  { title: '报废日期', sortable: true, width: '140', field: 'createTime', align: 'center' },
  { width: '120', title: '研磨单号', field: 'code', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入研磨单号' } },
  { width: '120', title: '供应商编码', field: 'supplierCode', align: 'center' },
  { width: '160', title: '供应商名称', field: 'supplierName', align: 'center' },
  { width: '100', title: '物料编码', field: 'materialCode', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入物料编码' } },
...
const getAddListRecord = async () => {
  ...
  // 钻咀研磨归还、 钻咀研磨报废
   if(currentAccountOrderId.value){
    ...
    rQueryParams2.type=DrillerOperationTypeEnum.ABRADE_RETURN;
    ...
    backQueryParams2.type = DrillerOperationTypeEnum.ABRADE_SCRAP;
    const res22 = await listDrillerAbradeRecord(rQueryParams2);
    const res33 = await listDrillerAbradeRecord(backQueryParams2);
    allData.value.checkedTabList1 = res22.rows;
    allData.value.checkedTabList2 = res33.rows;
  }

  rQueryParams.value.type = DrillerOperationTypeEnum.ABRADE_RETURN;
  backQueryParams.value.type = DrillerOperationTypeEnum.ABRADE_SCRAP;
  console.log("getAddListRecord getAddListRecord======", rQueryParams.value)
  const res2 = await listDrillerAbradeRecord({ ...rQueryParams.value, accountOrderId: currentAccountOrderId.value });
  const res3 = await listDrillerAbradeRecord({ ...backQueryParams.value, accountOrderId: currentAccountOrderId.value });
...
const doSave = async (resForm: any) => {
  ...
  //addForm.isTax = '1';
  addForm.drillerDetailBoList = addForm.detailList;
  addForm.drillerDetailBoList.forEach((it: any) => {
    it.drillerRecordId = it.id;
    it.drillerAbradeId = it.bizId;
    it.id = undefined;
  })
