        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="24">
            <el-form :model="queryParams" ref="queryFormRef" size="small" label-width="80px"
              ...
              <el-row>
                <el-col :span="6">
                  <el-form-item label="预收款单号" prop="code">
...
<!--                    <el-input v-model="queryParams.customerName"  clearable />-->
                    <el-select v-model="queryParams.customerName" style="width:100%"  clearable :collapse-tags="true" filterable>
                      <el-option
                ...
                <el-col :span="6">
                  <el-form-item label="收款日期" prop="cusId">
                ...
                <el-col :span="6">
                  <el-form-item label="单据类型" prop="businessType">
                    <el-select v-model="queryParams.businessType" style="width:100%"  clearable :collapse-tags="true" filterable>
                      <el-option
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="24" style="display:flex;justify-content: flex-end;">
            <el-button type="primary" style="float: right" @click="handleAdd" >新增预收款单</el-button>
          </el-col>
        </el-row>
      </template>
      <XTable
        toolId="prePaymentRecord"
        height="100%"
        :loading="loading"
      ...
      >
        ...
        <template #default-businessType="scope">
          <div v-for="(item,index) in typeList">
            <el-tag :key="index" :type="item.type" size="small" v-if="item.value == scope.row.businessType">{{item.label}}</el-tag>
          </div>
        </template>
        ...
        <template #default-payWay="scope">
          <dict-tag v-if="optionPayWayList.find((vo:any) => vo.value == scope.row.payWay)" :options="optionPayWayList" :value="scope.row.payWay" />
          <span v-else>--</span>
        </template>
        ...
        <template #default-make="scope">
          ...
          <el-button
            ...
            v-if="checkPermi(['financial:supplierPaymentManage:auditPreRepayment'])"
            >审核</el-button
          >
          <el-button
            link
            type="primary"
            v-show="scope.row.status == '4' && scope.row.businessType == '1'"
    ...
    <el-drawer v-model="paymentTable.visible" :title="paymentTable.title" destroy-on-close  size="80%" draggable>
      <template #header="{ close, titleId, titleClass }">
        <div>
          ...
          <el-tag style="margin-left: 2px" type="success" size="small" v-if="form?.businessType=='1'">手动新增</el-tag>
          <el-tag style="margin-left: 2px" type="success" size="small" v-if="form?.businessType=='2'">收款转存</el-tag>
          <el-tag style="margin-left: 2px" type="success" size="small" v-if="form?.businessType=='3'">账单转存</el-tag>
        </div>
      </template>
      <Payment :isSup="true" @getDetail="getDetail" @saveDraft="()=>{paymentTable.visible=false;getList();}" @submitDelete="handleDelete" :loading="buttonLoading" :isPre="true" :isUpdate="isUpdate" :readOnly="isReadOnly" :form="form" type="2" @row-click="handleRowClick" @submitPass = "submitPass" @submitReject="submitReject" @submit="submit"></Payment>
    </el-drawer>
    ...
    <el-drawer v-model="paymentTableDetail.visible" :title="paymentTableDetail.title" destroy-on-close  size="80%" draggable>
      <template #header="{ close, titleId, titleClass }">
        <div>
          ...
          <el-tag style="margin-left: 2px" type="success" size="small" v-if="detailInfo?.businessType=='2'">收款转存</el-tag>
          <el-tag style="margin-left: 2px" type="success" size="small" v-if="detailInfo?.businessType=='3'">账单转存</el-tag>
        </div>
      </template>
      <Payment :isSup="true" :isOpen="true" :loading="buttonLoading" :form="detailInfo" :isUpdate="true" :readOnly="true" type="2" ></Payment>
    </el-drawer>
  </div>
</template>

<script setup name="PreRepaymentList" lang="ts">
...
const typeList = ref([
  {label: '全部', value: '', elTagType: 'default', elTagClass: ''},
  {label: '手动新增', value: '1', elTagType: 'default', elTagClass: ''},
  {label: '收款转存', value: '2', elTagType: 'default', elTagClass: ''},
  {label: '账单转存', value: '3', elTagType: 'default', elTagClass: ''},
])
...
const columnList = ref([
  ...
  { title: '预收款单号', field: 'code', width: '160', align: 'center', filterType: undefined, filterParam: { placeholder: '请输入收款单号' } },
  ...
  { title: '单据类型', field: 'businessType', width: '90', align: 'center', filterType: undefined, filterParam: { placeholder: '请选择类型' }, filterData: () => typeList.value },
  { title: '收款日期', field: 'payDate', width: '140', align: 'center', filterType: undefined, filterParam: { placeholder: '请选择收款日期', valueFormat: 'YYYY-MM-DD'  } },
  { title: '客户名称', field: 'customerName', width: '100', align: 'center' , filterType: undefined},
  ...
  { title: '已用金额', field: 'writeOffPrice', width: '100', align: 'center'},
  { title: '未用金额', field: 'balance', width: '100', align: 'center'},
  ...
  { title: '来源单据', field: 'businessCode', width: '120', align: 'center' , filterType: 'input'},
...
const submit = (row: any) => {
  ...
  row.type = '2';
  addSubmitPrePaymentWriteOff(row).then(res => {
    proxy?.$modal.msgSuccess("操作成功");
    paymentTable.visible = false;
    getList();
  }).finally(() => {
...
const handleUpdate = async (row?: RepaymentRecordVO) => {
  ...
  const res2 = await getPaymentAccountList({isSup: '1',repayAllWriteOff : '0',customerId: form.value.customerId, paymentId: _id})
  form.value.confirmSelectList = res2.rows;
  console.log(form.value)
  // await nextTick();
  paymentTable.visible = true;
  paymentTable.title = "编辑收款单";
...
const handleDetail = async (row?: RepaymentRecordVO) => {
  ...
  const res2 = await getPaymentAccountList({isSup: '1',paymentId: _id, onlyPayment:'1',});
  form.value.confirmSelectList = res2.rows;
  console.log(form.value)
  paymentTable.visible = true;
  paymentTable.title = "预收款单详情";
  dialogTableLoading.value = false
...
const handleToExamine = async (row?: RepaymentRecordVO) => {
  ...
  isUpdate.value = true;
  // confirmSelectList.value = res.data.invoiceVoList;
  const res2 = await getPaymentAccountList({isSup: '1',repayAllWriteOff : '0',paymentId: _id, onlyPayment:'1',});
  form.value.confirmSelectList = res2.rows;
  paymentTable.visible = true;
  paymentTable.title = "收款审核";
  dialogTableLoading.value = false
}
...
const reloadUpdate = async (row?: RepaymentRecordVO) => {
  ElMessageBox.confirm(
    '确认重新修改后，该收款单在对应账单中的核销金额将回退，并进入待提交状态，是否继续?',
    '提示',
    {
...
/*详情*/
  const getDetail = async (id:any) =>{
    ...
    paymentTableDetail.title = "收款单详情";
    dialogTableLoading.value = false
  }
...
const handleDelete = async (row?: RepaymentRecordVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除收款单号为"' + row.code + '"的数据项？').finally(() => loading.value = false);
