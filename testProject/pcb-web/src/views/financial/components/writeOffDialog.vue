<template>
  <el-dialog v-model="openFlag" title="对账单冲销" width="60%" draggable @close="closeDialog">
    ...
    <XTable v-if="!isRepayment" :data="writeOffList" :pageShow="false" :columnList="props.titleColumnList?props.titleColumnList:writeOffSelectedColumnList" width border
    ...
    <XTable v-else-if="isRepayment && props.isSupplier" :data="writeOffList" :pageShow="false" :columnList="props.titleColumnList?props.titleColumnList:writeOffSelectedColumnList" width border
      :column-config="{ resizable: true }" max-height="300">
      <template #default-endTime="scope">
        <span>{{ parseTime(scope.row.endTime, '{y}-{m}-{d}') }}</span>
      </template>
    ...
    <XTable v-else :data="writeOffList" :pageShow="false" :columnList="props.titleColumnList?props.titleColumnList:custWriteOffSelectedColumnList" width border
      :column-config="{ resizable: true }" max-height="300">
      <template #default-endTime="scope">
        <span>{{ parseTime(scope.row.endTime, '{y}-{m}-{d}') }}</span>
      </template>
    ...
    <XTable v-if="isRepayment && props.isSupplier" :pageShow="false" :intervalCondition="intervalCondition" :data="repaymentRecordList"
      :columnList="supplierPaymentColumnList" ref="paymentTableRef" @checkbox-all="selectPaymentAllChangeEvent"
      @checkbox-change="selectPaymentChangeEvent" border @searchChange="searchChange" max-height="300"
      :column-config="{ resizable: true }">
      <template #default-payDate="scope">
        <span>{{ parseTime(scope.row.payDate, '{y}-{m}-{d}') }}</span>
...
import { writeOffAccountOrder, getPaymentList } from '@/api/financial/accountOrder';
...
const getPaymentRecordList = async () => {
  ...
  } else if (writeOffList?.value[0]?.customerId) {
