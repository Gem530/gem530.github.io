<template>
  <el-dialog v-model="openFlag" :title="writeOffDialog.title" width="90%"  draggable @close="closeDialog" custom-class="ptable-dialog">
    ...
    <XTable v-if="props.type=='1'" border :loading="loading" :data="writeOffRecordList" :columnList="columnList" style="height: 80%;" :pageShow="false" height="100%" class="ptable-content" :column-config="{ resizable: true }" >
...
<!--      </template>-->
    ...
    <XTable v-if="props.type=='2'" border :loading="loading" :data="writeOffRecordList" :columnList="custColumnList" :pageShow="false" :column-config="{ resizable: true }" height="100%" class="ptable-content">
...
const columnList = ref([
    ...
    { width: '160', title: '供应商', field: 'supplierName', align: 'center', },
...
const handleWriterOffRadioTable = async () => {
        ...
        if (writerOffRadioTable.value == "冲销记录") {
            loading.value = true;
            columnList.value[12].visible = true
           ...
           loading.value = true;
            columnList.value[12].visible = false
...
const setTypeTitle = () => {
    ...
    columnList.value[5].title = '应' + typeTitle.value + '金额';
    columnList.value[8].title = '已' + typeTitle.value + '金额';
    columnList.value[9].title =  typeTitle.value + '款单号';
