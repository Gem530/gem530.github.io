<template>
  <div class="p-2 xtable-page">
    <el-card shadow="never" class="xtable-card">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="20">
            <el-radio-group v-model="radioTable">
              ...
              <el-radio-button label="收款列表" @change="radioTableHandle" />
            </el-radio-group>
          </el-col>
      ...
      <XTable
        toolId="financialRepaymentRecord"
        height="100%"
      ...
      >
        ...
        <template #default-payAccount="scope">
          <dict-tag v-if="ownerId == '101'" :options="payAccountList" :value="scope.row.payAccount" />
    ...
    <el-drawer v-model="paymentTable.visible" :title="paymentTable.title" size="85%" draggable>
      <el-form
        border
        label-width="120px"
      ...
      >
        <el-row>
          <el-col  :span="8">
            <el-form-item size="small" label="收款类型:">
                ...
                <el-select :disabled="paymentTable.title?.includes('修改')" placeholder="请选择供应商" style="width: 100%;"
                  :default-first-option="true" v-model="isSup" @change="changeSup" >
                  <el-option :key="1" label="客户" :value="false"></el-option>
                  <el-option :key="2" label="供应商" :value="true"></el-option>
                </el-select>
          ...
          <el-col  :span="8">
            <el-form-item v-if="isSup" size="small" label="供应商:" prop="supplierId">
              <el-select :disabled="paymentTable.title?.includes('修改')" placeholder="请选择供应商" style="width: 100%;"
                  :default-first-option="true" v-model="form.supplierId" @change="changeSup"  filterable>
            ...
            <el-form-item v-if="!isSup" size="small" label="客户：" prop="customerId">
          ...
          <el-col :span="8">
            <el-form-item size="small" label="收款日期:" prop="payDate" style="width: 100%">
              <el-date-picker
          ...
          <el-col :span="8">
            <el-form-item size="small" label="收款金额:" prop="totalPrice">
              <el-input-number :disabled="paymentTable.title?.includes('详情')" min="0.01"
               :precision="2" style="width: 100%;" :controls="false" v-model="form.totalPrice" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item size="small" label="收款方式:" prop="payWay">
              <el-select v-model="form.payWay" clearable filterable placeholder="请选择收款方式" style="width: 100%;">
                <el-option v-for="item in payWayList" :key="item.value" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
          </el-col>
          <el-col :span="16">
            <el-form-item size="small" label="收款账户:" prop="payAccount">
              <el-select v-if="ownerId == '101'" v-model="form.payAccount" clearable filterable placeholder="请选择收款账户" style="width: 100%;" @change="changeAccount">
                <el-option v-for="item in payAccountList" :key="item.value"
                    :label="item.type +item.label + item.account" :value="item.value" />
              </el-select>
              <el-input v-else v-model="form.payAccount" maxlength="50" :rows="2" placeholder="请选择收款账户" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item size="small" label="收款账号:" prop="accountNumber">
              <el-input v-model="form.accountNumber" maxlength="100" :rows="2" placeholder="请输入收款账号" />
            </el-form-item>
          </el-col>
...
import { postListRepaymentRecord, getRepaymentRecord, delRepaymentRecord, addRepaymentRecord,
...
{label: 'The Hongkong and Shanghai Banking Corporation Limited', value: '4',bankNo:'053-484846-838(外币储蓄账户)',account:'，编号：004,代码：HSBCHKHHHKH',type:'香港公司美金账户，'},
{label: '企业微信', value: '5',account:'',bankNo:'',type:''},
...
const columnList = ref([
  ...
  { title: '收款单号', field: 'code', width: '160', align: 'center', filterType: 'input', filterParam: { placeholder: '请输入付款单号' } },
  { title: '收款日期', field: 'payDate', width: '140', align: 'center', filterType: 'intervalDate', filterParam: { placeholder: '请选择付款日期', valueFormat: 'YYYY-MM-DD'  } },
  { title: '客户编码', field: 'customerCodeIdList', width: '100', align: 'center',filterType: 'checkboxSearch' , filterData:()=>customerCodeList.value },
  { title: '客户名称', field: 'customerNameIdList', width: '100', align: 'center' ,filterType: 'checkboxSearch' , filterData:()=>customerNameList.value  },
  ...
  { title: '收款方式', field: 'payWay', width: '100', align: 'center' , filterType: 'checkboxButton', filterParam: { placeholder: '请选择付款方式' } ,filterData: () => payWayList.value },
  { title: '收款账号', field: 'accountNumber', width: '100', align: 'center' , filterType: 'input', filterParam: { placeholder: '请输入付款账户' } },
  { title: '收款账户', field: 'payAccount', width: '100', align: 'center'},
  { title: '收款金额', field: 'totalPrice', width: '100', align: 'center'},
  { title: '收款余额', field: 'balance', width: '100', align: 'center' ,  },
...
const paymentColumnList = ref([
    ...
    { width: '100', title: '收款金额', field: 'writeOffPrice', align: 'center', },
...
const addRules = ref<ElFormRules>(
  {
    ...
    //       if (!reg.test(value)) {
    //         callback(new Error('只能输入数字、字母或字符"_,~."'));
    //       } else {
    //         callback();
    //       }
    //     },
    ...
    accountNumber: [
      { required: true, message: "请选择收款账户或输入收款账号", trigger: "change" }
    ],
    remark: [
...
const checkPass = async (row: RepaymentRecordVO) => {
  await proxy?.$modal.confirm('是否确认提交收款单号为"' + row.code + '"的数据项？').finally(() => loading.value = false);
...
if(queryParams.value.payWay){
  ...
  const res = await postListRepaymentRecord(queryParams.value);
...
const changeSup = async () => {
  const _recordList = await queryPayRecordListByCustSup({type:'2',supplierId:form.value.supplierId})
    recordList.value = _recordList.rows;
}


const getListCust = async() => {
    const custRes = await listCompany();
