        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="24">
            <el-form :model="queryParams" ref="queryFormRef" size="small" label-width="80px"
              ...
              <el-row>
                <el-col :span="6">
                  <el-form-item label="预付款单号" prop="code">
...
<!--                    <el-input v-model="queryParams.supplierName"  clearable />-->
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="24" style="display:flex;justify-content: flex-end;">
            <el-button type="primary" style="float: right" @click="handleAdd" >新增预付款单</el-button>
          </el-col>
        </el-row>
      </template>
      <XTable
      ...
      >
        ...
        <template #default-make="scope">
          ...
          <el-button
            ...
            v-if="checkPermi(['financial:paymentManage:auditPrePayment'])"
            >审核</el-button
          >
          <el-button
            link
    ...
    <el-drawer v-model="paymentTable.visible" :title="paymentTable.title" destroy-on-close  size="80%" draggable>
      <template #header="{ close, titleId, titleClass }">
        <div>
          ...
          <el-tag style="margin-left: 2px" type="success" size="small" v-if="form?.businessType=='2'">付款转存</el-tag>
          <el-tag style="margin-left: 2px" type="success" size="small" v-if="form?.businessType=='3'">账单转存</el-tag>
        </div>
      </template>
      <Payment @getDetail="getDetail" @saveDraft="()=>{paymentTable.visible=false;getList()}" @submitDelete="handleDelete" :loading="buttonLoading" :isPre="true" :isUpdate="isUpdate" :readOnly="isReadOnly" :form="form" type="1" @row-click="handleRowClick" @submitPass = "submitPass" @submitReject="submitReject" @submit="submit"></Payment>
    </el-drawer>
...
<script setup name="PrePaymentList" lang="ts">
  ...
  import {
    ...
    addSubmitPrePaymentWriteOff, validatePaymentAccountDetail, approvePrePaymentCheckPer, rejectPaymentCheckPer
...
const typeList = ref([
  ...
  {label: '付款转存', value: '2', elTagType: 'default', elTagClass: ''},
...
{label: '支票', value: '9', elTagType: 'default', elTagClass: ''},
{label: '委托书', value: '10', elTagType: 'default', elTagClass: ''},
{label: '--', value: '', elTagType: 'default', elTagClass: ''},
]);
...
const columnList = ref([
  ...
  { title: '预付款单号', field: 'code', width: '160', align: 'center', filterType: undefined, filterParam: { placeholder: '请输入预付款单号' } },
...
const submit = (row: any) => {
  ...
  const submitPass = (id: any, isSup: boolean) => {
    buttonLoading.value = true;
    approvePrePaymentCheckPer({id: id, isSup: isSup}).then(res => {
      proxy?.$modal.msgSuccess("操作成功");
      paymentTable.visible = false;
      getList();
    }).finally(() => {
...
/*详情*/
  const getDetail = async (id:any) =>{
    ...
    paymentTableDetail.title = "付款单详情";
    dialogTableLoading.value = false
  }
...
const handleDelete = async (row?: RepaymentRecordVO) => {
  ...
  await proxy?.$modal.confirm('是否确认删除预付款单号为"' + row.code + '"的数据项？').finally(() => loading.value = false);
