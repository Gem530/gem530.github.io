...
<!--                    <el-input v-model="queryParams.customerName"  clearable />-->
      ...
      >
        ...
        <template #default-make="scope">
          ...
          <el-button
            ...
            v-if="checkPermi(['financial:paymentManage:auditPreRepayment'])"
            >审核</el-button
          >
          <el-button
            link
    ...
    <el-drawer v-model="paymentTable.visible" :title="paymentTable.title" destroy-on-close  size="80%" draggable>
      ...
      <Payment @getDetail="getDetail" @saveDraft="()=>{paymentTable.visible=false;getList();}" @submitDelete="handleDelete" :loading="buttonLoading" :isPre="true" :isUpdate="isUpdate" :readOnly="isReadOnly" :form="form" type="2" @row-click="handleRowClick" @submitPass = "submitPass" @submitReject="submitReject" @submit="submit"></Payment>
    </el-drawer>
    ...
    <el-drawer v-model="paymentTableDetail.visible" :title="paymentTableDetail.title" destroy-on-close  size="80%" draggable>
      ...
      <Payment :isOpen="true" :loading="buttonLoading" :form="detailInfo" :isUpdate="true" :readOnly="true" type="2" ></Payment>
    </el-drawer>
  </div>
</template>
...
const handleUpdate = async (row?: RepaymentRecordVO) => {
  ...
  const res2 = await getPaymentAccountList({repayAllWriteOff : '0',customerId: form.value.customerId, paymentId: _id})
...
const handleToExamine = async (row?: RepaymentRecordVO) => {
  ...
  isUpdate.value = true;
  // confirmSelectList.value = res.data.invoiceVoList;
  const res2 = await getPaymentAccountList({repayAllWriteOff : '0',paymentId: _id, onlyPayment:'1',});
  form.value.confirmSelectList = res2.rows;
  paymentTable.visible = true;
