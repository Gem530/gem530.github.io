...
<!--                @click="handleWriteOff">账单冲销</el-button>-->
          ...
          <XTable toolId="repayWriteOffAccountToolId1" v-model:page-size="accQueryParams.pageSize"
            ...
            <template #default-status="scope">
              <span v-if="scope.row.repayWriteOffPrice != 0">
                部分冲销
              </span>
...
<!--              <el-button link type="primary" @click="handleWriteOffOrder(scope.row)">收款冲销</el-button>-->
        ...
        <el-tab-pane label="已冲销列表" name="已冲销列表">
          ...
          <XTable toolId="repayWriteOffAccountToolId2" v-model:page-size="accQueryParams3.pageSize"
            ...
            :loading="loadingFinsih" :checkbox-config="{ trigger: 'row', checkMethod: selectableFun }"
            :row-style="tableRowClassName" :row-config="{ keyField: 'id' }">
            <template #default-code="scope">
              <span>{{ scope.row.code }}</span>
            </template>
            <template #default-status="scope">
...
<script setup name="RepayWriteOff" lang="ts">
...
const getAduitList = async () => {
  ...
  accQueryParams3.value.queryType = "1";
  accQueryParams3.value.customerCurrency = accQueryParams3.value.currency;
  accQueryParams3.value.customerMonthlyMethod = accQueryParams3.value.monthlyMethod;
...
const getList = async () => {
  ...
  accQueryParams.value.queryType = "1";
  accQueryParams.value.customerCurrency = accQueryParams.value.currency;
  accQueryParams.value.customerMonthlyMethod = accQueryParams.value.monthlyMethod;
...
const handleWriteOff = async () => {
  if (writeOffList.value.some(item => item.customerId == null || item.customerId == undefined || item.customerId == "")) {
    proxy?.$modal.msgError("冲销客户不能为空");
    return;
  }
  //判断writeOffList是不是同一个客户
...
/** 导出按钮操作 */
const handleExport = () => {
  let tool_tableName = 'repayWriteOffAccountToolId2';
  let queryInfo = {};
