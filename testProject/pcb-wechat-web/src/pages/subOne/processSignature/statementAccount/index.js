...
export default function ProcessSignatureStatementAccount() {
    const initRecord = {
        id: '',
        code: '',
        list: [],
    ...
    const getAuditList = (arr) => {
        ...
        // console.log(params)
        params.type = 5 // 类型（1：订单外协、2：采购、5：外协加工）），
    ...
    const openFile = async (e, item) => {
        e.stopPropagation()
        item.bizId = item.id
        GDownListRef.current?.openFile(e, item, '11')
    }
    ...
    return (
        <View className='statement-page'>
            ...
            <CSearch placeholder={'对账单号'} search={(e) => searchHandle(e)} type={11} scanPath={`/pages/reviewPdf/index`} params={`&type=11&title=对账单&isReview=2&code=''`} scanError="订单不存在"></CSearch>
