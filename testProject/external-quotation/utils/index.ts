...
// js如何判断国内的ip
export const isChineseIp = (ip: string) => {
    // 定义国内IP段范围
    var chinaIps = [
        /^10\./,                      // 10.x.x.x
...
export const setLocalStorage = (key: string, data: any) => {
    localStorage.setItem(key, encryptWithAes(JSON.stringify(data), env().VITE_ABC))
}

// 获取localStorage
export const getLocalStorage = (key: string) => {
    const localData = localStorage.getItem(key)
    return localData ? decryptWithAes(localData, env().VITE_ABC) : localData
}

// 表格数据导出excel
export const exportExcelByTable = (data: any[], headList: any[], sheet: string = 'sheet1', name: string = '导出') => {
    return new Promise((resolve, reject) => {
        // const { $loadingStart, $loadingEnd } = useNuxtApp()
        // $loadingStart({ text: '', background: 'rgba(0,0,0,0.6)' })
        // setTimeout(() => {
            try {
                let str = ''
                if (headList?.length) {
                    str += '<tr>'
                    headList.map((item: string) => {
                        str += `<td>${item}</td>`
                    })
                    str += '</tr>'
                }
                ...
                // 内容较少可以用这个
                // 因为浏览器URL长度限制，将excel数据转换成base64，数据量过大的时候长度超过浏览器GET限制，部分被阻断，识别不出来excel，下载失败
...
export const isIpad = () => {
    ...
    return /android|webos|ipad|playbook|silk|blackberry|bb10|tablet|kindle/.test(userAgent) || /mobi|android|iphone|ipad|phone|blackberry|opera mini|iemobile|windows phone/.test(userAgent);
}