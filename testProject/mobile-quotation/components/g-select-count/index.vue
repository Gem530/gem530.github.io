  ...
  <van-popup v-model:show="showSelect" round position="bottom">
    ...
    <van-radio-group v-model="checked" ref="checkboxGroupRef">
      <van-cell-group>
        ...
        >
          <template #title><slot name="title" :row="item">{{ item[dataFormat.text] }}</slot></template>
          <template #right-icon>
            <van-radio
...
<script lang="ts" setup name="GSelectCount">
const { t } = useI18n()
// import {
//   ref,
//   watch,
...
const initValue = () => {
  value.value = props.data.filter((v: any) => checked.value && checked.value.includes(v[dataFormat.value.value]))
...
const onConfirm = () => {
  if (checked.value) {
    ...
    const el = props.data.find((v: any) => v.value == checked.value)
    emits('confirm', el)
